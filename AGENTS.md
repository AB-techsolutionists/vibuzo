...
├── commands/               ← Each file = one command (subtask: true, "Do these steps NOW:")
...
├── opencode.jsonc          ← default_agent: vibuzo, permission rules
└── .opencode/              ← Installed output (gitignored — generated by installers)
```

## Critical Gotchas

- **Routing-only main files don't work.** `commands/session.md` and `commands/context.md` were removed — the agent reads them as plain text with no imperative instructions. Always use standalone sub-files with `subtask: true`.
- **Stale mirrors in `.opencode/commands/`** still contain `context.md` and `session.md` from before deletion. Sync `.opencode/` mirrors after any change to `commands/` or `agents/` (per `context/standards/source-mirror-synchronization.md`).
- **Install scripts reference deleted files.** `install.ps1` (lines 71-75) and `install.sh` (lines 56-61) download `context.md` and `session.md` from GitHub — these files no longer exist. Fix before next release.
...
- `subtask: true` tells opencode to run this via the task tool
...
1. Run `/session` to create `context/sessions/YYYY-MM-DD-<title>.md` with a generated summary
