...
- Establish a title-based compaction file format (`YYYY-MM-DD-<title>.md`) alongside the legacy single-file session format.
...
- All compactions use the standard anchored summary format (Goal, Constraints, Progress, Decisions, Next Steps, Critical Context, Relevant Files)

## Progress
### Done
- Full specification delivered for three subcommands:
  - `/session` (zero-arg) — auto-generate, save, and timeline-update
  - `/session view <ref>` — view past compactions by exact ref, date, or natural language keywords (yesterday, today, last, recent, all)
...
- Compaction format defined with all 8 sections (Goal, Constraints & Preferences, Progress, Key Decisions, Next Steps, Critical Context, Relevant Files)
...
## Key Decisions
- Title-based compaction files (`YYYY-MM-DD-<title>.md`) replace single-file daily sessions and opaque sequence numbers — enables multiple compactions per day with descriptive names
...
## Critical Context
- This is the **first title-based compaction** — no `YYYY-MM-DD-<title>.md` files existed prior to this one
...
- `context/index.md` already references `sessions/index.md` as "Auto-generated master timeline of all compactions"
...
- `context/sessions/`: Target directory for all new title-based compaction files
