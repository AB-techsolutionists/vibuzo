...
2. **Standard prompt format** — always render inside a code block so opencode displays it as a terminal card. Use this exact format:
...
6. **Plan approval** — after presenting an implementation plan (inline or structured), ask "Approve this plan? (y/N)" before executing any part of it.
7. **Delegation gate** — before spawning Deepveloper via /spec at level ≥ 2, present the task description and ask for approval.
