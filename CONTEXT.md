...
### Deepveloper
The npm CLI tool (`deepveloper`) that detects AI coding tools, writes agent definitions, and guides the user through installing skills. It is run interactively with `npx deepveloper@latest`.
...
- **opencode**: `.opencode/agents/deepveloper.md` — YAML frontmatter (`mode: primary`, `hidden: false`) + System Prompt body
...
Deepveloper does not write these; the skills setup step (`/setup-matt-pocock-skills`) fills in the skills section.
_Avoid_: project instructions

### Matt Pocock Skills
A collection of engineering workflow skills (code-review, domain-modeling, TDD, grilling, implement, etc.) distributed via `npx skills@latest add mattpocock/skills`. The user installs them manually after Deepveloper's flow; the CLI prints the command.
...
1. Clear the screen and show banner
...
5. Show post-install guidance: how to install Matt Pocock's Skills (`npx skills@latest add mattpocock/skills`) and run `/setup-matt-pocock-skills`
